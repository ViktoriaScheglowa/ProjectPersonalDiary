...
class OffensiveWordValidator:
    ...
    def __init__(self, forbidden_words=None, comments=None):
        self.forbidden_words = (
            forbidden_words if forbidden_words is not None else ["плохо", "ужасно"]
        )
        self.comments = comments or _(
            "Коментарий содержит запрещенное слово: '%(word)s'."
        )
        ...
        for word in self.forbidden_words:
            if word in value_lower:
                raise ValidationError(
                    self.comments, params={"word": word}, code="offensive_content"
                )

    def __eq__(self, other):
        return (
            isinstance(other, self.__class__)
            and self.forbidden_words == other.forbidden_words
